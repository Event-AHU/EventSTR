...
class EventOCRDataset(BasePromptDataset):  # 新增的数据集
    def __init__(self, vis_processor, text_processor, vis_root, ann_path):
        super().__init__(vis_processor, text_processor, vis_root, ann_path)

        self.prompts = ["What is the text in the image?"]
    ...
    def __getitem__(self, index):
        ...
        text_input = self.prompts[0]

        answer = ann["answers"][0]
        if len(ann["answers"]) > 1:
            answer_weight = {}
            for ans in ann["answers"]:
                if ans in answer_weight:
                    answer_weight[ans] += 1 / len(ann["answers"])
                else:
                    answer_weight[ans] = 1 / len(ann["answers"])
