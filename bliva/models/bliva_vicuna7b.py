...
from bliva.models.memory import Memory
...
class BLIVAVicuna(Blip2Base):
    ...
    def __init__(
        ...
        self.shape_similar_dict = self.load_shape_similar_dict('/wangx/BLIVA/bliva/data/shape_similar_dict.txt')
        ...
        self.memory = Memory(num_patterns=128, pattern_dim=128, feature_dim=1408)
    ...
    def load_shape_similar_dict(self, file_path):
        shape_similar_dict = {}
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                words = line.strip().split('，')
                for word in words:
                    shape_similar_dict[word] = [w for w in words if w != word]
        return shape_similar_dict

    def is_chinese_char(self, char):
        """判断是否为中文字符"""
        return '\u4e00' <= char <= '\u9fff'
    def get_candidate_words(self, decoded_text):
        candidate_words = set()  # 使用 set 来确保候选词不重复
        tokens = re.findall(r'[\u4e00-\u9fff]|[a-zA-Z]+', decoded_text)
        ...
        # 分类字符为中文和英文
        for token in tokens:
            if all(self.is_chinese_char(c) for c in token):  # 如果是中文字符
                chinese_chars.update(token)  # 直接按字去重
            else:
                english_words.add(token)  # 英文按单词去重

        # 一次性获取中文字符的形近字
        for char in chinese_chars:
            candidate_words.update(self.shape_similar_dict.get(char, []))

        # 一次性获取英文单词的相似词
        for word in english_words:
            candidate_words.update(self.shape_similar_dict.get(word.lower(), []))

        return list(candidate_words)  # 返回去重后的候选词列表
    # def get_candidate_words(self, decoded_text):
    #     candidate_words = set()  # 使用 set 来确保候选词不重复
    #     tokens = re.findall(r'[\u4e00-\u9fff]|[^\u4e00-\u9fff]+', decoded_text)

    #     for token in tokens:
    #         if all(self.is_chinese_char(c) for c in token):
    #             for char in token:
    #                 similar_words = self.shape_similar_dict.get(char, [])
    #                 candidate_words.update(similar_words)  # 使用 set 的 update 方法直接去重
    #         # 其他非中文字符可以根据需求决定是否添加
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image)) #torch.Size([2, 257, 1408]) #torch.Size([2, 3, 32, 128])
            image_features= self.visual_encoder.get_intermediate_layers(image)[-2] # [batch_size, 257, 1408]
            
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        
        image_features = image_features[:, 1:]  #torch.Size([2, 256, 1408])
        image_features = self.memory(image_features)
        ...
        llm_tokens = self.llm_tokenizer(
            ...
            padding="longest",
            return_tensors="pt"
        ).to(image.device) #与train不同，train将label加上了
        ...
        #第二阶段
        candidate_word_lists = [self.get_candidate_words(text) for text in output_text]

        new_text_input = [
        f"Original text: {text}, candidate words: {', '.join(candidate_words)}, please correct the incorrect words."
        for text, candidate_words in zip(output_text, candidate_word_lists)
        ]
        #对新的prompt进行Embeddings
        ...
        #将输入和输出的 文本token 进行拼接，得到用于 LLM 的 token 和对应的掩码（llm阶段）
        ...
        # do not apply loss to the padding 将填充的 文本token 用 -100 进行掩码，这样模型在计算损失时不会考虑这些位置
        targets = llm_tokens['input_ids'].masked_fill(
            llm_tokens['input_ids'] == self.llm_tokenizer.pad_token_id, -100
        )

        # do not apply loss to the text input (i.e., instruction) 对于输入部分的 token 也不进行损失计算，因此将它们的 `targets` 值设为 -100。
        for i, l in enumerate(input_part_targets_len):
            targets[i][:l] = -100

        # do not apply loss to the query tokens 为查询向量的 token 位置创建一个全为 -100 的张量，确保这些位置不被用于计算损失
        empty_targets = (
            torch.ones(atts_llm.size(), dtype=torch.long).to(image.device).fill_(-100)
        )
        # do not apply loss to the additional image features 为附加的局部视觉特征 token 创建一个全为 -100 的张量，确保这些位置不被用于计算损失
        empty_add_targets = (
            torch.ones(atts_add_feature_llm.size(), dtype=torch.long).to(image.device).fill_(-100)
        )
        #targets = torch.cat([empty_targets, targets], dim=1)
        targets = torch.cat([empty_targets, empty_add_targets, targets], dim=1) #将query、附加局部视觉特征的 `targets` 与拼接后的文本输入-输出 `targets` 合并，得到最终的 `targets`

        inputs_embeds = self.llm_model.get_input_embeddings()(llm_tokens['input_ids']) #使用 LLM 模型的嵌入层将 文本token id 转换为嵌入向量
        #inputs_embeds = torch.cat([inputs_llm, inputs_embeds], dim=1)
        #attention_mask = torch.cat([atts_llm, llm_tokens['attention_mask']], dim=1)
        inputs_embeds = torch.cat([inputs_llm, add_feature_llm, inputs_embeds], dim=1) #将 query、局部视觉特征嵌入与文本嵌入进行拼接，作为 LLM 的最终输入
        attention_mask = torch.cat([atts_llm, atts_add_feature_llm, llm_tokens['attention_mask']], dim=1)
